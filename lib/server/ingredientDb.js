...
import { searchLocalizedFoods, hasStrongLocalizedMatch } from '../nutrition/localizedFoodSearch.js';
...
async function searchSupabaseIngredients(query, limit = 15) {
  ...
  if (error) {
    const { data: fallback } = await supabase
      ...
      .select('*')
      .limit(0);
    return (fallback || []).map((r) => normalizeIngredient({
      id: r.id,
      name: r.name,
      variant: r.variant,
      per100g: r.per_100g,
...
export async function searchIngredients(query, { limit = 40, mealParse = false } = {}) {
  const raw = String(query || '').trim();
  const q = mealParse ? normalizeMealSearchQuery(raw) : raw;
