const LOCALIZED_FOODS = [
  {
    ...
    defaultGrams: 86,
    servingOptions: [
      { id: 'slice', label: '1 Slice (~250 kcal)', grams: 86, calories: 250, protein: 10, carbs: 27, fat: 11 },
  ...
  },
  {
    ...
    defaultGrams: 92,
    servingOptions: [
      { id: 'slice', label: '1 Slice (~225 kcal)', grams: 92, calories: 225, protein: 9, carbs: 29, fat: 7 },
  ...
  },
  {
    ...
    defaultGrams: 95,
    servingOptions: [
      { id: 'slice', label: '1 Slice (~261 kcal)', grams: 95, calories: 261, protein: 11.4, carbs: 27.6, fat: 11.4 },
  ...
  {
    id: 'lidl-crownfield-oats',
    name: 'Crownfield Rolled Oats',
    brand: 'Lidl Crownfield',
    searchTerms: ['lidl oats', 'lidl ovaz', 'lidl ovăz', 'crownfield oats', 'crownfield ovaz', 'fulgi de ovaz lidl'],
    per100g: { calories: 370, protein: 13, carbs: 58, fat: 7, fiber: 10 },
    defaultGrams: 40,
    servingOptions: [
      { id: 'serving', label: '40g Dry Serving', grams: 40, calories: 148, protein: 5.2, carbs: 23.2, fat: 2.8 },
      { id: '100g', label: '100g', grams: 100, calories: 370, protein: 13, carbs: 58, fat: 7 },
    ],
  },
  {
...
function scoreFood(food, query) {
  ...
  const terms = (food.searchTerms || []).map(normalize);
  ...
  let score = 0;
  if (name === q) score += 500;
  if (terms.some((term) => term === q)) score += 480;
  ...
  if (specificTokens.some((token) => name.includes(token) || terms.some((term) => term.includes(token)))) score += 240;
  if (specificTokens.some((token) => !haystack.includes(token))) score -= 180;
  return score;
}
...
export function getLocalizedFoodById(id) {
  const rawId = String(id || '').replace(/^localized-/, '');
  const food = LOCALIZED_FOODS.find((entry) => entry.id === rawId);
  if (!food) return null;
  return normalizeFood(food, 200);
}
