...
import { findCorrectionForQuery, resolveLineWithUserContext } from './mealUserContext.js';
...
function enrichMatched(item, foodName, line, grams, choices = {}) {
  const category = detectCategory(foodName) || detectCategory(item.name);
  let per100g = { ...item.per100g };

  const corrected =
    getUserCorrection(item.id)
    || findCorrectionForQuery(foodName, choices.__userContext?.userCorrections || [])
    || findCorrectionForQuery(item.name, choices.__userContext?.userCorrections || []);
...
export async function parseDescribeLine(line, deps, choices = {}) {
  ...
  if (choice?.ingredientId && deps.getIngredientById) {
    ...
    if (item) return enrichMatched(item, foodName, line, choice.grams ?? weight, choices);
  }

  if (!choice?.ingredientId) {
    const contextHit = resolveLineWithUserContext(line, choices);
  ...
  if (!results?.length) {
    ...
    if (prep && !choice) {
      ...
      if (retry?.length) {
        ...
        if (ranked.length) return enrichMatched(ranked[0].item, foodName, line, weight, choices);
      }
    }
  ...
  if (!choice?.ingredientId) {
    ...
    if (prep && top.item.variant && top.item.variant !== prep && top.score < 95) {
      ...
      if (better && better.score >= top.score - 15) {
        return enrichMatched(better.item, foodName, line, weight, choices);
      }
  ...
  if (!choice?.ingredientId && top.item.source === 'localized-nutrition' && top.item.servingOptions?.[0] && !/\b\d+(?:[.,]\d+)?\s*(g|gram|grams|kg|ml|l|oz)\b/i.test(line)) {
    ...
    return {
      ...enrichMatched(top.item, foodName, line, (serving.grams || weight) * count, choices),
  ...
  return enrichMatched(top.item, foodName, line, choice?.grams ?? weight, choices);
}
